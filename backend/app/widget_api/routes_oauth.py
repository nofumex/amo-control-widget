...
@router.get("/install")
async def oauth_install(settings: Settings = Depends(get_settings)) -> RedirectResponse:
...
async def oauth_callback(
    ...
    base_url = _base_url_from_referer(referer)
    ...
    tenant = await _upsert_tenant(session, account_id, subdomain, base_url, settings.integration_mode)
    ...
    if existing is None:
        existing = OAuthToken(tenant_id=tenant.id, access_token_encrypted="", refresh_token_encrypted="", expires_at=token.expires_at(300))
...
async def oauth_refresh(tenant_id: int, settings: Settings = Depends(get_settings), session: AsyncSession = Depends(get_session)) -> dict:
    tenant = await session.get(Tenant, tenant_id)
    token_row = (await session.scalars(select(OAuthToken).where(OAuthToken.tenant_id == tenant_id))).first()
    if tenant is None or token_row is None:
...
async def _upsert_tenant(session: AsyncSession, account_id: int | None, subdomain: str, base_url: str, mode: str) -> Tenant:
